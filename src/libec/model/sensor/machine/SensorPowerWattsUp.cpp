...
namespace cea
{
  ...
  WattsUpPowerMeter::experiment()
  {
    ...
    DebugLog::writeMsg(DebugLog::INFO, "WattsUpMeter", ("Checking device file: " + string(wu_device)));

    ret = stat(wu_device, &s);
    if (ret != 0)
    {
      DebugLog::writeMsg(DebugLog::ERROR, "WattsUpMeter", ("stat() device file: " + string(wu_device)));
      return;
    }
    if (!S_ISCHR(s.st_mode))
    {
    ...
    if (status != NO_ERROR)
    {
      ...
      DebugLog::writeMsg(DebugLog::ERROR, "WattsUpMeter", errorType + " :: " + errorMsg);
      return;
    }
