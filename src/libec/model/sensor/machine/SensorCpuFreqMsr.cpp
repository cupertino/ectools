...
#include <libec/sensor/SensorCpuFreqMsr.h>
...
namespace cea
{
  ...
  // Static Members
  ///////////////////////////////////////////////////////////////////
  const char* CpuFreqMsr::ClassName = "CpuFreqMsr";

  ///////////////////////////////////////////////////////////////////
  // Public Members
  ///////////////////////////////////////////////////////////////////
  CpuFreqMsr::CpuFreqMsr(unsigned short cpu_id)
  {
    clean();
    ...
    _name = "CPU" + cpuIdStr + "_FREQUENCYMSR";
    _alias = "CPU" + cpuIdStr + "_FREQ_MSR";
    _cpuId = cpu_id;
    _type = U64;
    ...
    if (!SystemInfo::isRootUser())
      {
        DebugLog::writeMsg(DebugLog::ERROR, "CpuFreqMsr::CpuFreqMsr()",
  ...
  CpuFreqMsr::~CpuFreqMsr()
  {
    clean();
  }

  inline const char*
  CpuFreqMsr::getClassName()
  {
    return ClassName;
  }
  ...
  {
    mperf_stop();
    mperf_get_count_freq(&_cValue.U64, _cpuId);
...
#define MSR_AMD_HWCR 0xc0010015
   ...
   */
  int
  CpuFreqMsr::init_maxfreq_mode(void)
  {
    int ret;
    ...
    use_sysfs: if (cpufreq_get_hardware_limits(0, &min, &max_frequency))
      {
        DebugLog::writeMsg(DebugLog::ERROR, "CpuFreqMsr::init_maxfreq_mode()",
  ...
  void
  CpuFreqMsr::mperf_register(void)
  {
   ...
   */
  int
  CpuFreqMsr::read_msr(int cpu, unsigned int idx, unsigned long long *val)
  {
    int fd;
  ...
  int
  CpuFreqMsr::mperf_get_tsc(unsigned long long *tsc)
  {
    int ret;
    ret = read_msr(0, MSR_TSC, tsc);
    if (ret)
      DebugLog::writeMsg(DebugLog::ERROR, "CpuFreqMsr::mperf_get_tsc()",
          "Reading TSC MSR failed.");
    return ret;
  }

  int
  CpuFreqMsr::mperf_init_stats(unsigned int cpu)
  {
  ...
  int
  CpuFreqMsr::mperf_measure_stats(unsigned int cpu)
  {
    unsigned long long val;
    int ret;
  ...
  long long
  CpuFreqMsr::timespec_diff_us(struct timespec start, struct timespec end)
  {
  ...
  int
  CpuFreqMsr::mperf_get_count_freq(unsigned long long *count, unsigned int cpu)
  {
  ...
  void
  CpuFreqMsr::mperf_unregister(void)
  {
  }
